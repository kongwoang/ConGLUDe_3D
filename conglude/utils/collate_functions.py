...
def custom_collate(batch):
    ...
    has_ligand_3d = len(batch[0]) == 5
    if has_ligand_3d:
        proteins, ligands, labels, ligand_idx, ligands_3d = zip(*batch)
    else:
    ...
    if type(proteins[0]) == list:
        ...
        if has_ligand_3d and ligands_3d[0] is not None:
            ligands_3d_batch = ligands_3d[0]
        else:
            ligands_3d_batch = None
    ...
    # Standard batching (one protein per dataset item)
        ...
        if ligands[0] is not None:
            ...
            if has_ligand_3d and ligands_3d[0] is not None:
                ligands_3d_batch = {
                    "coords": torch.cat([item["coords"] for item in ligands_3d], dim=0),
                    "atom_types": torch.cat([item["atom_types"] for item in ligands_3d], dim=0),
                    "atom_mask": torch.cat([item["atom_mask"] for item in ligands_3d], dim=0),
                    "num_atoms": torch.cat([item["num_atoms"] for item in ligands_3d], dim=0),
                    "success": torch.cat([item["success"] for item in ligands_3d], dim=0),
                }
            else:
                ligands_3d_batch = None
    ...
    if has_ligand_3d:
        return proteins_batch, ligands_batch, labels_batch, ligand_batch_idx_batch, ligand_idx_batch, ligands_3d_batch
