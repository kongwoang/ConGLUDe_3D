...
class LigandProcessor:
    ...
    def __init__(self, 
        ...
        self.conformer_random_seed = conformer_random_seed
    ...
    @staticmethod
    def _get_ligand3d_paths(dataset_dir: str) -> Dict[str, str]:
        base_dir = os.path.join(dataset_dir, "processed", "ligand_embeddings")
        return {
            "coords": os.path.join(base_dir, "ligand3d_coords.pt"),
            "atom_types": os.path.join(base_dir, "ligand3d_atom_types.pt"),
            "atom_mask": os.path.join(base_dir, "ligand3d_atom_mask.pt"),
            "num_atoms": os.path.join(base_dir, "ligand3d_num_atoms.pt"),
            "success": os.path.join(base_dir, "ligand3d_success.pt"),
            "metadata": os.path.join(base_dir, "metadata_ligand3d.json"),
        }
    ...
    def process(self):
        ...
        # Build one 3D conformer per ligand and keep row alignment with index2smiles.
        mols_with_h = []
        num_atoms = []
        success = []
        conf_ids = []
        for smiles in smiles_list:
            ...
            n_atoms = int(mol_h.GetNumAtoms())
            num_atoms.append(n_atoms)

            if n_atoms == 0:
                mols_with_h.append(mol_h)
                success.append(False)
                conf_ids.append(-1)
                continue

            params = AllChem.ETKDGv3()
            params.randomSeed = int(self.conformer_random_seed)
            ...
            mols_with_h.append(mol_h)
            success.append(True)
            conf_ids.append(int(cid))

        a_max = int(max(num_atoms)) if len(num_atoms) > 0 else 0
        n_ligands = len(smiles_list)

        coords = torch.zeros((n_ligands, a_max, 3), dtype=torch.float32)
        atom_types = torch.zeros((n_ligands, a_max), dtype=torch.long)
        atom_mask = torch.zeros((n_ligands, a_max), dtype=torch.bool)
        num_atoms_tensor = torch.tensor(num_atoms, dtype=torch.long)
        success_tensor = torch.tensor(success, dtype=torch.bool)

        for i, mol_h in enumerate(mols_with_h):
            n_atoms = int(num_atoms[i])
            if mol_h is None or n_atoms == 0:
                continue

            # Atomic number + 1 reserves 0 as the padding token.
            for j, atom in enumerate(mol_h.GetAtoms()):
                atom_types[i, j] = int(atom.GetAtomicNum()) + 1
                atom_mask[i, j] = True

            if success[i]:
                conf = mol_h.GetConformer(conf_ids[i])
                conf_coords = conf.GetPositions()
                coords[i, :n_atoms] = torch.tensor(conf_coords, dtype=torch.float32)

        ligand3d_paths = self._get_ligand3d_paths(self.dataset_dir)
        torch.save(coords, ligand3d_paths["coords"], pickle_protocol=4)
        torch.save(atom_types, ligand3d_paths["atom_types"], pickle_protocol=4)
        torch.save(atom_mask, ligand3d_paths["atom_mask"], pickle_protocol=4)
        torch.save(num_atoms_tensor, ligand3d_paths["num_atoms"], pickle_protocol=4)
        torch.save(success_tensor, ligand3d_paths["success"], pickle_protocol=4)
        write_json(
            ligand3d_paths["metadata"],
            {
                "num_ligands": n_ligands,
                "max_atoms": a_max,
                "atom_type_encoding": "atomic_number_plus_one",
                "padding_atom_type": 0,
                "num_conformers": 1,
                "conformer_random_seed": int(self.conformer_random_seed),
            },
        )
