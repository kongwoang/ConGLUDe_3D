...
class ConGLUDeDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_ligand_3d = use_ligand_3d
        self.memmap = memmap
        ...
        if (fingerprint_type is not None or load_descriptors) and calc_mol_feats:
            ligand3d_paths = [
                os.path.join(self.dataset_dir, "processed", "ligand_embeddings", "ligand3d_coords.pt"),
                os.path.join(self.dataset_dir, "processed", "ligand_embeddings", "ligand3d_atom_types.pt"),
                os.path.join(self.dataset_dir, "processed", "ligand_embeddings", "ligand3d_atom_mask.pt"),
                os.path.join(self.dataset_dir, "processed", "ligand_embeddings", "ligand3d_num_atoms.pt"),
                os.path.join(self.dataset_dir, "processed", "ligand_embeddings", "ligand3d_success.pt"),
            ]
            missing_ligand3d = self.use_ligand_3d and not all(os.path.isfile(path) for path in ligand3d_paths)

            if not os.path.isfile(self.fingerprint_path) or not os.path.isfile(self.descriptor_path) or (not os.path.isfile(self.ligand_metadata_path) and self.memmap) or missing_ligand3d:
            ...
            if self.use_ligand_3d:
                self.ligand_3d_features = self.load_ligand_3d_data()
    ...
    def load_ligand_3d_data(
        self
    ) -> Optional[dict]:
        ligand_dir = os.path.join(self.dataset_dir, "processed", "ligand_embeddings")
        paths = {
            "coords": os.path.join(ligand_dir, "ligand3d_coords.pt"),
            "atom_types": os.path.join(ligand_dir, "ligand3d_atom_types.pt"),
            "atom_mask": os.path.join(ligand_dir, "ligand3d_atom_mask.pt"),
            "num_atoms": os.path.join(ligand_dir, "ligand3d_num_atoms.pt"),
            "success": os.path.join(ligand_dir, "ligand3d_success.pt"),
        }
        ...
        return {
            "coords": torch.load(paths["coords"], weights_only=False).to(dtype=torch.float32),
            "atom_types": torch.load(paths["atom_types"], weights_only=False).long(),
            "atom_mask": torch.load(paths["atom_mask"], weights_only=False).bool(),
            "num_atoms": torch.load(paths["num_atoms"], weights_only=False).long(),
            "success": torch.load(paths["success"], weights_only=False).bool(),
        }
    ...
    def get(
        ...
        Tuple[object, Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor], Optional[dict]],
    ]:
        """
        ...
        if not (self.fingerprint_type is None and self.load_descriptors is False):
            ...
            if self.use_ligand_3d and hasattr(self, "ligand_3d_features") and self.ligand_3d_features is not None:
                ligands_3d = {k: v[ligand_idx] for k, v in self.ligand_3d_features.items()}
            else:
                ligands_3d = None
        ...
        if self.use_ligand_3d:
            return protein, ligands, labels, ligand_idx, ligands_3d
    ...
    def get_multi_pdb_targets(
        ...
        Tuple[List[object], torch.Tensor, torch.Tensor, torch.Tensor, Optional[dict]],
    ]:
        """
        ...
        if self.use_ligand_3d:
            return proteins, ligands, labels, ligand_idx, ligands_3d
