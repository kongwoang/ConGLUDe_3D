...
class ConGLUDeModel(pl.LightningModule):
    ...
    def forward(
        ...
        ligands_3d: Dict[str, torch.Tensor] = None,
    ) -> Dict[str, Any]:
        """
        ...
        if not ligands is None:
            if ligands_3d is not None:
                try:
                    encoded_ligands = self.ligand_encoder(ligands, ligands_3d=ligands_3d)
                except TypeError:
    ...
    def process_step(
        ...
        batch: Union[
            ...
            Tuple[Batch, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]],
        ],
        dataset_specs: Dict[str, Any],
    ) -> torch.Tensor:
        """
        ...
        if len(batch) == 6:
            proteins, ligands, labels, ligand_batch_idx, ligand_idx, ligands_3d = batch
        else:
            proteins, ligands, labels, ligand_batch_idx, ligand_idx = batch
            ligands_3d = None

        output = self.forward(proteins, ligands, ligand_batch_idx, ligand_idx, dataset_specs, ligands_3d=ligands_3d)
